...
def evaluation(type, dataset, batch_size):
    ...
    probs = torch.ones_like(indices).float().cpu()
    probs_type_size_in_bytes = 4
    ...
    for indptr_cache, indices_cache, prob_cache in zip(indptr_cache_set,
                                                       indices_cache_set,
                                                       prob_cache_set):
        avg_sample_time = bench(indptr, indices, probs, train_nid, batch_size,
                                type_size_in_bytes, probs_type_size_in_bytes,
                                indptr_cache, indices_cache, prob_cache)
        if local_rank == 0:
            print(
                "World Size {} | Device {} | Dataset {} | Type {} | indptr cache size {:.1f} | indices cache size {:.1f} | sampling time {:.3f} ms"
                .format(comm_size, torch.ops.dgs_ops._CAPI_get_rank(), dataset,
                        type, indptr_cache, indices_cache, avg_sample_time))

    torch.ops.dgs_ops._CAPI_finalize()


def bench(indptr, indices, probs, train_nid, batch_size, type_size,
          probs_type_size_in_bytes, indptr_cache, indices_cache, prob_cache):
    ...
    chunk_probs = torch.classes.dgs_classes.ChunkTensor(
        probs,
        int((probs.numel() * probs_type_size_in_bytes * prob_cache) /
            torch.ops.dgs_ops._CAPI_get_size()) + probs_type_size_in_bytes)
